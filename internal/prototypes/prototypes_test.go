...
func TestLoadRejectsSymlinkedCapabilityBoundary(t *testing.T) {
	root := newSaga(t)
	outside := t.TempDir()
	if err := os.Symlink(outside, filepath.Join(root, "___features", "core.feature", "___requirements")); err != nil {
...
func TestHTMLPrototypeIsImmutableDigestVerifiedPackage(t *testing.T) {
	...
	created, err := AddHTML(root, "test", AddHTMLInput{Feature: "core", ID: "checkout", RevisionID: "r1", Title: "Checkout", State: StateReady, SourcePath: source, CreatedAt: testTime})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExternalAndAllowlistedEmbedSources(t *testing.T) {
	root := newSaga(t)
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "figma-link", RevisionID: "r1", Title: "Figma", State: StateDraft, URL: "https://www.figma.com/file/abc", CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "figma-embed", RevisionID: "r1", Title: "Embedded Figma", State: StateReady, URL: "https://www.figma.com/file/abc", EmbedURL: "https://embed.figma.com/proto/abc", Allowlist: allowlist, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	doc, err := Load(root, "test")
	if err != nil {
	...
	_, err = AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "bad", RevisionID: "r1", Title: "Bad", State: StateReady, URL: "https://safe.example/fallback", EmbedURL: "https://embed.figma.com/proto/abc", Allowlist: &bad, CreatedAt: testTime})
	if err == nil || !strings.Contains(err.Error(), "exactly match") {
		t.Fatalf("origin error = %v", err)
	}
	_, err = AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "implicit", RevisionID: "r1", Title: "Implicit", State: StateReady, URL: "https://safe.example/fallback", EmbedURL: "https://embed.figma.com/proto/abc", CreatedAt: testTime})
...
func TestUnresolvedAnnotationsAreQualityGapsUntilComposition(t *testing.T) {
	...
	annotation, err := AddAnnotation(root, "test", AddAnnotationInput{Feature: "core", ID: "buy-button", Prototype: prototype, Target: story, Rationale: "Shows the primary action.", PrototypeRevision: prototypeRevision, StoryRevision: storyRevision, Selector: Selector{Kind: SelectorElement, ElementID: "buy"}, CreatedAt: testTime})
	if err != nil {
	...
	if _, err := AddHTML(root, "test", AddHTMLInput{Feature: "core", ID: "checkout", RevisionID: "r1", Title: "Checkout", State: StateReady, SourcePath: source, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	doc, err = Load(root, "test")
	if err != nil {
...
func TestRevisionPinsBecomeStaleWithoutRetargeting(t *testing.T) {
	...
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "flow", RevisionID: "r1", Title: "Flow", State: StateReady, URL: "https://example.com/flow", CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	story := "urn:change-saga:test:story:buyer"
	if _, err := AddAnnotation(root, "test", AddAnnotationInput{Feature: "core", ID: "flow-link", Prototype: prototype, Target: story, Rationale: "Shows flow.", PrototypeRevision: prototype + ":revision:r1", StoryRevision: story + ":revision:s1", Selector: Selector{Kind: SelectorProvider, ProviderID: "node-1"}, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
...
func TestRepositoryStylesArePinnedAndFailClosed(t *testing.T) {
	...
	if _, err := AddHTML(root, "test", AddHTMLInput{Feature: "core", ID: "styled", RevisionID: "r1", Title: "Styled", State: StateReady, SourcePath: html, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
...
func TestAnnotationIdentityIsScopedToPrototype(t *testing.T) {
	...
	for _, prototypeID := range []string{"one", "two"} {
		...
		if _, err := AddAnnotation(root, "test", AddAnnotationInput{Feature: "core", ID: "same", Prototype: prototype, Target: story, Rationale: "Scoped edge.", PrototypeRevision: prototype + ":revision:r1", StoryRevision: story + ":revision:s1", Selector: Selector{Kind: SelectorRegion, Region: &Region{X: 0, Y: 0, Width: 1, Height: 1}}, CreatedAt: testTime}); err != nil {
			t.Fatal(err)
		}
	}
	doc, err := Load(root, "test")
...
func TestSelectorValidationAndSourceCompatibility(t *testing.T) {
	...
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "external", RevisionID: "r1", Title: "External", State: StateReady, URL: "https://example.com", CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	story := "urn:change-saga:test:story:story"
	_, err := AddAnnotation(root, "test", AddAnnotationInput{Feature: "core", ID: "bad-region", Prototype: prototype, Target: story, Rationale: "Bad region.", PrototypeRevision: prototype + ":revision:r1", StoryRevision: story + ":revision:s1", Selector: Selector{Kind: SelectorRegion, Region: &Region{X: .9, Y: 0, Width: .2, Height: .2}}, CreatedAt: testTime})
	if err == nil || !strings.Contains(err.Error(), "normalized") {
		t.Fatalf("region error = %v", err)
	}
	if _, err := AddAnnotation(root, "test", AddAnnotationInput{Feature: "core", ID: "html-selector", Prototype: prototype, Target: story, Rationale: "Incompatible after composition.", PrototypeRevision: prototype + ":revision:r1", StoryRevision: story + ":revision:s1", Selector: Selector{Kind: SelectorText, ExactText: "Hello"}, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	doc, err := Load(root, "test")
	if err != nil {
...
func TestPrototypesLoadFromEveryFeatureAndCreatesNameTheirFeature(t *testing.T) {
	root := newSaga(t)
	mustFeature(t, root, "billing")
	if _, err := AddExternal(root, "test", AddExternalInput{ID: "implied", RevisionID: "r1", Title: "Implied", State: StateDraft, URL: "https://example.com", CreatedAt: testTime}); err == nil || !strings.Contains(err.Error(), "a feature is required") {
		t.Fatalf("implied feature error = %v", err)
	}
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "missing", ID: "unknown", RevisionID: "r1", Title: "Unknown", State: StateDraft, URL: "https://example.com", CreatedAt: testTime}); err == nil || !strings.Contains(err.Error(), "does not exist") {
		t.Fatalf("unknown feature error = %v", err)
	}
	input := AddExternalInput{Feature: "billing", ID: "invoice", RevisionID: "r1", Title: "Invoice", State: StateReady, URL: "https://example.com/invoice", CreatedAt: testTime, RequestID: "req-1"}
	created, err := AddExternal(root, "test", input)
	if err != nil {
		t.Fatal(err)
	}
	if created.Path != "___features/billing.feature/___requirements/prototypes/invoice.prototype" {
	...
	if _, err := AddExternal(root, "test", other); err == nil || !strings.Contains(err.Error(), "already exists") {
		t.Fatalf("replay into another feature error = %v", err)
	}
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "cart", RevisionID: "r1", Title: "Cart", State: StateReady, URL: "https://example.com/cart", CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || revised.Path != "___features/billing.feature/___requirements/prototypes/invoice.prototype/revisions/r2.revision" {
	...
	wrong := annotation
	wrong.Feature = "core"
	if _, err := AddAnnotation(root, "test", wrong); err == nil || !strings.Contains(err.Error(), "is in feature") {
		t.Fatalf("annotation into another feature error = %v", err)
	}
	annotated, err := AddAnnotation(root, "test", annotation)
	if err != nil || annotated.Path != "___features/billing.feature/___requirements/prototypes/annotations/invoice.prototype/pay.json" {
	...
	if len(doc.Prototypes) != 2 || doc.Prototypes[0].Identity.ID != "cart" || doc.Prototypes[0].Feature != "core" || doc.Prototypes[1].Feature != "billing" {
		t.Fatalf("prototypes = %#v", doc.Prototypes)
	}
	if len(doc.Annotations) != 1 || doc.Annotations[0].Feature != "billing" {
...
func TestPrototypeIDsAreUniqueAcrossTheApp(t *testing.T) {
	root := newSaga(t)
	mustFeature(t, root, "billing")
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "core", ID: "shared", RevisionID: "r1", Title: "Shared", State: StateReady, URL: "https://example.com", CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
	if _, err := AddExternal(root, "test", AddExternalInput{Feature: "billing", ID: "shared", RevisionID: "r1", Title: "Shared", State: StateReady, URL: "https://example.com", CreatedAt: testTime}); err == nil || !strings.Contains(err.Error(), "already exists") {
		t.Fatalf("second feature create error = %v", err)
	}
	// Simulate a Git merge that brings the same ID into a second feature.
	source := filepath.Join(root, "___features", "core.feature", "___requirements", "prototypes", "shared.prototype")
	target := filepath.Join(root, "___features", "billing.feature", "___requirements", "prototypes", "shared.prototype")
...
func TestFeatureContentAtAppRootIsRejected(t *testing.T) {
	root := newSaga(t)
	mustMkdir(t, filepath.Join(root, "___requirements"))
	if _, err := Load(root, "test"); err == nil || !strings.Contains(err.Error(), "belongs in a feature") {
...
func newSaga(t *testing.T) string {
	...
	mustFeature(t, root, "core")
	return root
}

func mustFeature(t *testing.T, root, id string) {
	t.Helper()
	if _, err := applayout.WriteFeature(root, applayout.FeatureManifest{ID: id, Title: id, CreatedAt: testTime}); err != nil {
		t.Fatal(err)
	}
}
