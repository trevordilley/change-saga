...
func TestV3DesignChapterAndFragmentEnterExistingRenderTree(t *testing.T) {
	...
	writeDesignTestFile(t, filepath.Join(serverFeatureDir(root), "overview.fragment", "fragment.json"), `{"version":2,"id":"overview","title":"Overview","media_type":"text/markdown","entrypoint":"content.md"}`)
	writeDesignTestFile(t, filepath.Join(serverFeatureDir(root), "overview.fragment", "content.md"), "Narrative overview.\n")
	writeDesignTestFile(t, filepath.Join(serverFeatureDir(root), "___design", "architecture.chapter", "chapter.json"), `{"version":2,"id":"architecture","title":"Technical architecture"}`)
	writeDesignTestFile(t, filepath.Join(serverFeatureDir(root), "___design", "architecture.chapter", "sequence.fragment", "fragment.json"), `{"version":2,"id":"sequence","title":"Request sequence","media_type":"text/markdown","entrypoint":"content.md"}`)
	writeDesignTestFile(t, filepath.Join(serverFeatureDir(root), "___design", "architecture.chapter", "sequence.fragment", "content.md"), "# Request sequence {#request-sequence}\n\nRenderable design content.\n")
