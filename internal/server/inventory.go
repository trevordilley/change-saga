...
func (a *app) documentationPage(w http.ResponseWriter, r *http.Request) {
	...
	for index, member := range revision.Components {
		...
		view.Members = append(view.Members, documentationMember{Pin: member, Name: name, Status: inventory.LinkStatus(member), Y: index*100 + 20})
	}
	...
	view.Code = a.documentationCode(r.Context(), requirements.References(revision.Code), record.Kind)
	var body bytes.Buffer
...
<p class="trace-kind">{{.Kind}}</p><h2>{{.Name}}</h2>
{{if ne .Status "current"}}<p role="status" class="gap" data-documentation-status="{{.Status}}">This reference is {{.Status}}. You are reading the saved revision; it has not been repinned.</p>{{with .CurrentPin}}<button type="button" data-documentation-target="{{.Target}}" data-documentation-revision="{{.Revision}}">Read current definition</button>{{end}}{{end}}
<p class="documentation-prose">{{.Explanation}}</p>
{{if .Members}}<h3>Component interactions</h3><svg class="documentation-diagram" viewBox="0 0 430 {{.Height}}" role="group" aria-label="Directed component interactions; explanations and exact code follow."><defs><marker id="documentation-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor"/></marker></defs>{{range .Edges}}<path d="{{.Path}}" fill="none" stroke="currentColor" stroke-width="2" marker-end="url(#documentation-arrow)"><title>{{.From}} → {{.To}}: {{.Description}}</title></path>{{end}}{{range .Members}}<a href="/api/documentation?target={{.Pin.Target}}&amp;revision={{.Pin.Revision}}" data-documentation-target="{{.Pin.Target}}" data-documentation-revision="{{.Pin.Revision}}" aria-label="Open {{.Name}}"><g transform="translate(20 {{.Y}})"><rect width="230" height="50" fill="var(--bg,white)" stroke="currentColor"/><text x="12" y="30" font-size="14" fill="currentColor">{{.Name}}</text></g></a>{{end}}</svg>
<ul class="documentation-members">{{range .Members}}<li><button type="button" data-documentation-target="{{.Pin.Target}}" data-documentation-revision="{{.Pin.Revision}}">{{.Name}}</button>{{if ne .Status "current"}} <span class="gap">{{.Status}}</span>{{end}}</li>{{end}}</ul>
...
<details><summary>Definition history</summary><p>Viewing <code>{{.Revision}}</code>. Opening another revision does not update this slide.</p><ul>{{range .History}}<li><button type="button" data-documentation-target="{{.Target}}" data-documentation-revision="{{.Revision}}">{{.Revision}}</button></li>{{end}}</ul></details>
</article>{{end}}
`
