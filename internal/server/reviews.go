...
{{define "review-item-affordance"}}<span class="landmark-affordance review-item-affordance" data-landmark-affordance><button type="button" class="review-item-link" data-open-diffs="review-item-{{.DOMID}}" aria-label="{{if .Diffs}}Open {{len .Diffs}} code {{if eq (len .Diffs) 1}}reference{{else}}references{{end}}{{else}}Open details{{end}} for {{.Item.Label}}">{{if .Diffs}}Code · {{len .Diffs}}{{else}}Details{{end}}</button>{{if .RecordHref}}<button type="button" class="review-item-link" data-open-stories="review-record-{{.DOMID}}" aria-label="Open affected documentation for {{.Item.Label}}">Affected · {{.RecordKind}}</button>{{end}}</span>{{end}}
