...
func (a *app) reviewPage(w http.ResponseWriter, r *http.Request) {
	...
	for index, slide := range review.Deck.Slides {
		slideView := &reviewSlideView{
			...
			Interactive: slide.MediaType == "text/html", Position: index + 1, Report: slideReports[slide.ID],
		}
		...
		for _, item := range slide.Items {
			itemView := &reviewItemView{Item: item, DOMID: domID(item.Target), Threads: threadViewsFor(threads, item.Target, review.ID, a.mutationToken, view.Frozen)}
			if item.Record != "" {
...
{{define "review-page"}}<div class="review-surface" data-review="{{.Review.ID}}"><header class="review-top"><nav class="requirements-breadcrumbs" aria-label="Review breadcrumb"><a href="/reviews">Reviews</a><span>/</span><strong>{{.Review.Title}}</strong></nav><div class="review-title-row"><div><h1>{{.Review.Title}}</h1>{{with .Review.PullRequest}}<p class="review-pr">{{if .URL}}<a href="{{.URL}}">{{if .Number}}Pull request #{{.Number}}{{else}}{{.URL}}{{end}}</a>{{else}}Pull request #{{.Number}}{{end}}</p>{{end}}</div><details class="review-source"><summary>Source and currency</summary>{{template "review-range" .Report}}</details></div><p class="review-rule">{{if .Frozen}}This review is history: its change has landed. It is shown exactly as it was reviewed.{{else}}Review one slide at a time. Decisions apply to the complete active slide; comments stay attached to the exact slide or Item.{{end}}</p></header>
<div class="review-workspace" data-review-workspace><p class="review-live" aria-live="polite" data-review-live></p><nav class="review-slide-nav" aria-label="Review slides"><ol>{{range .Slides}}<li><a href="#{{.DOMID}}" data-review-slide-link="{{.Slide.ID}}"><span class="review-slide-thumb" aria-hidden="true">{{if .Interactive}}<iframe tabindex="-1" sandbox="allow-scripts" src="{{.VisualURL}}" title=""></iframe>{{else}}<img loading="lazy" src="{{.VisualURL}}" alt="">{{end}}</span><span class="review-slide-nav-copy"><small>Slide {{.Position}}</small><strong>{{.Slide.Title}}</strong><span class="review-slide-nav-state">{{range .Report.Decisions}}<span>{{reviewState .State}}{{if eq .Currency "out_of_date"}} · out of date{{end}}</span>{{else}}<span>Not reviewed</span>{{end}}{{if .Report.OpenThreads}}<span>{{.Report.OpenThreads}} open {{if eq .Report.OpenThreads 1}}thread{{else}}threads{{end}}</span>{{end}}</span></span></a></li>{{end}}</ol></nav><div class="review-stage"><div class="review-stagebar" aria-label="Slide navigation"><span data-review-position></span><div><button type="button" data-review-previous>Previous</button><button type="button" data-review-next>Next</button></div></div><div class="review-main">{{$page := .}}{{range .Slides}}<section class="review-slide{{if .OutOfDate}} out-of-date{{end}}" id="{{.DOMID}}" data-review-slide="{{.Slide.ID}}"><header class="review-slide-head"><p class="review-slide-kicker">Slide {{.Position}} of {{len $page.Slides}}</p><h2 tabindex="-1">{{.Slide.Title}}</h2><p class="review-takeaway">{{.Slide.Takeaway}}</p></header><div class="review-slide-body"><div class="review-visual">{{if .Interactive}}<iframe sandbox="allow-scripts" src="{{.VisualURL}}" title="{{.Slide.Title}}"></iframe>{{else}}<img src="{{.VisualURL}}" alt="{{.Slide.Title}}">{{end}}</div>
<aside class="review-decisions" aria-label="Decisions on {{.Slide.Title}}"><h3>Slide decision</h3><ul>{{range .Report.Decisions}}<li class="review-decision-row {{.State}}{{if eq .Currency "out_of_date"}} out-of-date{{end}}" data-decision-state="{{.State}}" data-currency="{{.Currency}}"><strong>{{reviewState .State}}</strong> by {{reviewer .}} at <code>{{short .Commit}}</code>{{if eq .Currency "out_of_date"}} <span class="review-out-of-date" data-out-of-date>Out of date</span>{{else if eq .Currency "unknown"}} <span class="review-unknown">currency unknown</span>{{end}}{{if .Reasons}}<ul class="review-reasons">{{range .Reasons}}<li>{{.}}</li>{{end}}</ul>{{end}}{{if .Body}}<p class="review-body">{{.Body}}</p>{{end}}</li>{{else}}<li class="review-decision-row none">No decision yet</li>{{end}}</ul>{{if not $page.Frozen}}<details class="review-decision-editor"><summary>Review this slide</summary><form class="review-decision-form" method="post" action="/reviews/{{$page.Review.ID}}/decision" data-review-decision-form="{{.Slide.ID}}"><input type="hidden" name="token" value="{{$page.MutationToken}}"><input type="hidden" name="slide" value="{{.Slide.ID}}"><label><span>Note <small>required for changes</small></span><textarea name="body" rows="3"></textarea></label><div class="review-decision-buttons"><button type="submit" name="state" value="approved" data-review-approve>Approve slide</button><button type="submit" name="state" value="changes_requested" data-review-request-changes>Request changes</button><button type="submit" name="state" value="none" data-review-withdraw>Withdraw</button></div></form></details>{{end}}</aside></div>
<details class="review-slide-details"><summary><span>Slide details</span><small>{{len .Items}} {{if eq (len .Items) 1}}Item{{else}}Items{{end}}{{if .Report.OpenThreads}} · {{.Report.OpenThreads}} open {{if eq .Report.OpenThreads 1}}thread{{else}}threads{{end}}{{end}}</small></summary><div class="review-slide-details-body"><div class="review-items">{{range .Items}}<article class="review-item" id="{{.DOMID}}" data-review-item="{{.Item.ID}}"><h3>{{.Item.Label}}</h3><p>{{.Item.Description}}</p>{{if .RecordHref}}<p class="review-record">Documentation: <a href="{{.RecordHref}}" data-review-record="{{.Item.Record}}">{{.RecordLabel}}</a></p>{{end}}{{range .Diffs}}{{template "review-diff" .}}{{end}}{{template "review-threads" .Threads}}{{template "review-comment-form" (reviewCommentForm $page .Item.Target .Item.Label)}}</article>{{end}}</div>
<div class="review-slide-threads">{{template "review-threads" .Threads}}{{template "review-comment-form" (reviewCommentForm $page .Slide.Target .Slide.Title)}}</div></div></details></section>{{end}}</div></div></div></div>{{end}}
...
.review-top{padding:0 4px}.review-top h1{margin:8px 0 2px}.review-title-row{display:flex;align-items:flex-start;justify-content:space-between;gap:24px}.review-title-row .review-pr{margin:0}.review-range code,.review-decisions code{font-size:12px}
.review-rule{max-width:780px;color:var(--muted,#555)}.review-diagnostic{display:block;color:#a15c00}.review-source{flex:0 0 auto;margin-top:8px}.review-source summary,.review-compose summary,.review-decision-editor summary{cursor:pointer;color:var(--accent);font-weight:600}.review-source .review-range{max-width:520px;margin:8px 0 0;padding:8px 10px;border:1px solid var(--line,#ddd);background:var(--surface,#fff)}
...
.review-workspace{display:grid;grid-template-columns:240px minmax(0,1fr);gap:24px;align-items:start;margin-top:20px}.review-live{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.review-slide-nav{position:sticky;top:56px;max-height:calc(100vh - 72px);overflow:auto;border-right:1px solid var(--line,#ddd);padding-right:12px}.review-slide-nav ol{list-style:none;margin:0;padding:0}.review-slide-nav li+li{border-top:1px solid var(--line,#ddd)}.review-slide-nav a{display:grid;grid-template-columns:72px minmax(0,1fr);gap:10px;padding:10px 8px;color:inherit;text-decoration:none;border-left:3px solid transparent}.review-slide-nav a:hover,.review-slide-nav a:focus-visible{background:var(--bg-subtle)}.review-slide-nav a[aria-current="step"]{border-left-color:var(--accent);background:var(--bg-subtle)}.review-slide-thumb{display:block;aspect-ratio:16/9;overflow:hidden;border:1px solid var(--line,#ddd);background:#fff}.review-slide-thumb img,.review-slide-thumb iframe{width:100%;height:100%;object-fit:contain;border:0;pointer-events:none}.review-slide-nav-copy{min-width:0}.review-slide-nav-copy small,.review-slide-nav-copy strong,.review-slide-nav-state,.review-slide-nav-state>span{display:block}.review-slide-nav-copy small,.review-slide-nav-state{font-size:11px;color:var(--muted,#666)}.review-slide-nav-copy strong{font-size:13px;line-height:1.25;margin:1px 0 3px}.review-stage{min-width:0;max-width:1120px}.review-stagebar{position:sticky;top:44px;z-index:5;display:flex;justify-content:space-between;align-items:center;min-height:42px;padding:6px 0;background:var(--bg,#fff);border-bottom:1px solid var(--line,#ddd)}.review-stagebar>div{display:flex;gap:6px}.review-stagebar button{min-width:84px}.review-stagebar button:disabled{opacity:.45}.review-slide{border:1px solid var(--line,#ddd);border-radius:12px;padding:18px;margin:16px 0 40px}.review-slide[hidden]{display:none}
.review-slide.out-of-date{border-color:#b45309}
.review-slide-kicker{margin:0;color:var(--muted,#666);font-size:12px;text-transform:uppercase;letter-spacing:.06em}.review-slide-head h2{margin:3px 0}.review-takeaway{margin-top:4px;color:var(--muted,#555)}.review-slide-body{display:grid;grid-template-columns:minmax(0,3fr) minmax(240px,1fr);gap:18px;align-items:start}
...
.review-decisions{position:sticky;top:104px;border-left:1px solid var(--line,#ddd);padding-left:16px}.review-decisions h3{margin:0 0 6px}.review-decisions ul{list-style:none;padding:0;margin:0}.review-decision-row{margin:6px 0}.review-decision-editor{margin-top:12px}.review-decision-form{margin-top:8px}
...
.review-slide-details{border-top:1px solid var(--line);margin-top:18px;padding-top:12px}.review-slide-details>summary{display:flex;justify-content:space-between;gap:16px;cursor:pointer;font-weight:600;color:var(--accent)}.review-slide-details>summary small{font-weight:400;color:var(--muted)}.review-slide-details-body{padding-top:6px}.review-item{border-top:1px solid var(--line,#eee);padding-top:14px;margin-top:18px}.review-item>h3{margin-bottom:2px}.review-item>p{margin-top:2px}
...
@media(max-width:1050px){.review-workspace{grid-template-columns:1fr}.review-slide-nav{position:relative;top:auto;max-height:none;overflow-x:auto;border-right:0;border-bottom:1px solid var(--line,#ddd);padding:0 0 8px}.review-slide-nav ol{display:flex;min-width:max-content}.review-slide-nav li{width:220px}.review-slide-nav li+li{border-top:0;border-left:1px solid var(--line,#ddd)}.review-slide-nav a{grid-template-columns:64px minmax(0,1fr)}.review-stagebar{top:44px}}
@media(max-width:720px){.review-title-row{display:block}.review-source{margin:8px 0}.review-stagebar{top:42px}.review-stagebar button{min-width:0}.review-slide{border-left:0;border-right:0;border-radius:0;margin-left:-12px;margin-right:-12px;padding:14px 12px}.review-slide-body{grid-template-columns:1fr}.review-decisions{position:static;border-left:0;border-top:1px solid var(--line,#ddd);padding:12px 0 0}.review-decision-buttons button{flex:1 1 140px}.review-slide-nav li{width:200px}}
@media(prefers-reduced-motion:reduce){.review-slide-nav a{scroll-behavior:auto}}
`
