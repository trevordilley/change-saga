...
{{define "review-item-affordance"}}<span class="landmark-affordance review-item-affordance" data-landmark-affordance><button type="button" class="review-item-link" data-short="{{if .Diffs}}Code{{else}}Details{{end}}" data-open-diffs="review-item-{{.DOMID}}" aria-label="{{if .Diffs}}Open {{len .Diffs}} code {{if eq (len .Diffs) 1}}reference{{else}}references{{end}}{{else}}Open details{{end}} for {{.Item.Label}}">{{if .Diffs}}Code · {{len .Diffs}}{{else}}Details{{end}}</button>{{if .RecordHref}}<button type="button" class="review-item-link" data-short="{{.RecordKind}}" data-open-stories="review-record-{{.DOMID}}" aria-label="Open affected documentation for {{.Item.Label}}">Affected · {{.RecordKind}}</button>{{end}}</span>{{end}}
...
.review-deck-slide .landmark-hotspot{border-color:#d3941888;background:#f2bd4b0d}.review-deck-slide .landmark-hotspot>.review-item-affordance{opacity:1}
.review-item-affordance{gap:2px;padding:3px;background:var(--landmark-affordance-bg)}.review-item-link{min-height:24px;padding:3px 7px;border:1px solid var(--line);border-radius:4px;background:var(--bg);color:var(--accent);font:650 10px var(--ui);white-space:nowrap;box-shadow:0 1px 2px #0002}.review-item-link:hover,.review-item-link:focus-visible{background:var(--accent-soft);border-color:var(--accent)}
...
.review-annotation-status{position:fixed;z-index:75;left:50%;bottom:62px;max-width:min(520px,calc(100vw - 24px));margin:0;padding:7px 11px;border:1px solid var(--line);border-radius:7px;background:var(--bg);box-shadow:var(--shadow);color:var(--ink);font:600 12px var(--ui);transform:translateX(-50%)}.review-annotation-status.failed{border-color:#b45309;color:var(--ink)}.review-annotation-status[hidden]{display:none}
