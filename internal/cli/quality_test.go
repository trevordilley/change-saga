...
func TestQualityCommandsAuthorTheFrozenRecordsEndToEnd(t *testing.T) {
	...
	added := runQuality(t, definition, "test-case", "add", root, "--from", "-")
	...
	if replay := runQuality(t, definition, "test", "add", root, "--from", "-"); !replay.Replayed {
	...
	runQuality(t, "", "policy", "set", root, "--criterion", "urn:change-saga:checkout:story:refund:criterion:cutoff",
...
func TestQualityMutationFailureReportsJSONAndWritesNothing(t *testing.T) {
	...
	err := Quality(context.Background(), []string{
		"test-case", "add", root, "--id", "deadline", "--title", "No kinds", "--automation", "manual",
	...
	if _, statErr := os.Lstat(filepath.Join(root, quality.RootDir)); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("failed add adopted quality: %v", statErr)
	}
	output.Reset()
	if err := qualityCommand(context.Background(), []string{"test-case", "add", root, "--from", "-"}, &output, strings.NewReader(`{"id":"x","score":1}`)); err == nil || !strings.Contains(err.Error(), "unknown field") {
