...
func TestPrototypeAddHTMLCopiesAnImmutableRevisionAndReplays(t *testing.T) {
	...
	args := []string{
		"add-html", root, "--id", "checkout", "--revision", "r1", "--title", "Checkout",
	...
	if created.Path != "___requirements/prototypes/checkout.prototype" {
	...
	packaged, err := os.ReadFile(filepath.Join(root, "___requirements", "prototypes", "checkout.prototype", "revisions", "r1.revision", "html", "index.html"))
...
func TestPrototypeAuthoringDoesNotRequireAnnotationOrLinkage(t *testing.T) {
	...
	created := runPrototype(t, "add-html", root, "--id", "explore", "--revision", "r1",
	...
	if err := Story(context.Background(), []string{
		"add", root, "--id", "buyer", "--revision", "s1", "--event", "proposed", "--title", "Buyer",
...
func TestPrototypeAddExternalRequiresExplicitEmbedAllowlisting(t *testing.T) {
	root := newLivingSaga(t)
	reference := runPrototype(t, "add-external", root, "--id", "figma-link", "--revision", "r1",
	...
	embed := runPrototype(t, "add-external", root, "--id", "figma-embed", "--revision", "r1",
	...
	err = Prototype(context.Background(), []string{
		"add-external", root, "--id", "implicit", "--revision", "r1", "--title", "Implicit",
	...
	output.Reset()
	err = Prototype(context.Background(), []string{
		"add-external", root, "--id", "stray", "--revision", "r1", "--title", "Stray",
...
func TestPrototypeReviseIsAppendOnlyAndReconcilesHeads(t *testing.T) {
	root := newLivingSaga(t)
	source := newPrototypeSource(t, "<!doctype html><button id=\"buy\">Buy</button>")
	runPrototype(t, "add-html", root, "--id", "checkout", "--revision", "r1", "--title", "Checkout", "--source", source)
	...
	original, err := os.ReadFile(filepath.Join(root, "___requirements", "prototypes", "checkout.prototype", "revisions", "r1.revision", "html", "index.html"))
...
func TestValidateCoversPrototypeRecords(t *testing.T) {
	...
	packaged := filepath.Join(root, "___requirements", "prototypes", "checkout.prototype", "revisions", "r1.revision", "html", "index.html")
