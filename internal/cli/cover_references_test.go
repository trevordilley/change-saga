...
func fileEventSaga(t *testing.T) (root, repo, base, head string) {
	...
	writeFile(t, filepath.Join(root, "overview.fragment", "content.md"), "# Events {#events}\n\nEvery kind of file change.\n")
...
// digest of the whole file, and it alone covers a file event.
func TestCoverFileReferencesTheWholeFileOnASide(t *testing.T) {
	...
	for _, atom := range report.Uncovered {
		if atom.Path == "service/added.go" || atom.Path == "service/deleted.go" {
			t.Fatalf("a whole-file reference must cover its event and every line on its side; %v is uncovered", atom)
		}
	}
...
// file exists, which also account for that side's lines.
func TestCoverChangedLinesCompletesEveryFileEvent(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"service/added.go", []string{"new service/added.go file"}},
		{"service/deleted.go", []string{"old service/deleted.go file"}},
		{"service/after.go", []string{"new service/after.go file"}},
		// Either path of an edited rename selects the rename event and the
		// deleted line at the merge-base.
		{"service/moved.go", []string{"new service/relocated.go file", "old service/moved.go 3-3"}},
	} {
...
func TestCoverBatchRecordSpellsEveryReferenceFlag(t *testing.T) {
	...
	var result coverageMutationOutput
	if err := json.Unmarshal([]byte(output), &result); err != nil || result.Records != 5 || result.References != 5 {
	...
	for name, want := range map[string]coderef.Reference{
		...
		"changed":    {Commit: base, Path: "service/deleted.go", Digest: sha256Digest(deletedFile)},
	} {
