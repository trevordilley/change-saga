...
func TestQueryDispatchesEveryOperationAndPreservesArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{"context", []string{"--feature", "core", "--expand", "story-1", "--limit", "24"}, livingQuery{Operation: "context", Filters: livingapp.Filters{Feature: "core", Expand: "story-1"}, Limit: 24}},
		{"personas", []string{"--persona", "user", "--cursor", "persona-page", "--limit", "12"}, livingQuery{Operation: "personas", Filters: livingapp.Filters{Persona: "user"}, Cursor: "persona-page", Limit: 12}},
		{"persona-references", []string{"--persona", "user", "--cursor", "reference-page", "--limit", "13", "--conflict-cursor", "conflict-page", "--conflict-limit", "7"}, livingQuery{Operation: "persona-references", Filters: livingapp.Filters{Persona: "user"}, Cursor: "reference-page", Limit: 13, ConflictCursor: "conflict-page", ConflictLimit: 7}},
		{"term-references", []string{"--term", "basket", "--cursor", "term-page", "--limit", "14", "--conflict-cursor", "term-conflicts", "--conflict-limit", "8"}, livingQuery{Operation: "term-references", Filters: livingapp.Filters{Term: "basket"}, Cursor: "term-page", Limit: 14, ConflictCursor: "term-conflicts", ConflictLimit: 8}},
		{"requirements", []string{"--feature", "core", "--requirement", "story-1", "--state", "accepted", "--cursor", "c9", "--limit", "25"}, livingQuery{Operation: "requirements", Filters: livingapp.Filters{Feature: "core", Requirement: "story-1", State: "accepted"}, Cursor: "c9", Limit: 25}},
...
func TestQueryRejectsAdversarialArgumentsBeforeOpening(t *testing.T) {
	...
	tests := []struct {
		...
		{"zero conflict limit", []string{"persona-references", "--saga", "x", "--persona", "user", "--conflict-limit", "0"}},
		{"huge conflict limit", []string{"term-references", "--saga", "x", "--term", "basket", "--conflict-limit", "1001"}},
		{"empty conflict cursor", []string{"persona-references", "--saga", "x", "--persona", "user", "--conflict-cursor", ""}},
		...
		{"missing context feature", []string{"context", "--saga", "x"}},
		{"ambiguous context page and expansion", []string{"context", "--saga", "x", "--feature", "core", "--expand", "story", "--cursor", "cursor"}},
