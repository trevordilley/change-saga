...
func TestInstalledSkillRoutesFocusedTasks(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "feature interview before a candidate story", fixture: "Interview me about reusable architecture, using the personas already in change.saga before drafting stories.",
...
var skillCommands = map[string]func(context.Context, []string, io.Writer) error{
	...
	"add-slide": AddSlide, "apply-slide": ApplySlide, "diagram": Diagram, "set-slide-content": SetSlideContent, "add-item": AddItem, "add-section": AddSection,
