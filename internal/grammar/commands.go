...
var commands = []Command{
	...
	},
	{
		...
		Summary: "create a story identity, its initial complete revision, and its proposed lifecycle event; criteria may wait until behavior is confirmed",
		Flags: []Flag{featureCreateFlag, repeatable("persona", "URN", "persona URN the story serves: the \"As a ...\" of its statement; optional", false),
			...
			repeatable("criterion", "ID=STATEMENT", "independent pass/fail acceptance criterion; use only confirmed behavior", false), repeatable("citation", "URN", "citation URN", false),
	...
	},
	{
		...
		Summary: "append a story lifecycle event: proposed, accepted, deferred, rejected, or retired; accepted requires every current revision head to have a criterion",
	...
	},
	{
		...
		Flags: []Flag{featureIDFlag,
			...
			required("id", "ID", "stable criterion id; removed ids can never be reused"), required("statement", "TEXT", "one independent, observable pass/fail obligation"),
			requestIDFlag, fromFileFlag, jsonFlag,
		},
		Positionals: sagaOnly,
	},
	...
	{
		Name: "relation repin", Status: StatusImplemented, Mutates: true, Writes: []string{"relation-repin"},
		Usage:   "change-saga relation repin --relation URN --rationale TEXT [--id ID] [--to-revision URN] [--from-revision URN] [--to-content-digest DIGEST] [--from-content-digest DIGEST] [--feature ID] [--request-id ID] [--json] <saga>",
		Summary: "confirm one relation still holds and advance its pins; the relation keeps its id and every revision it was read against, and omitted pins default to the heads it is behind",
		Flags: []Flag{featureIDFlag,
			required("relation", "URN", "canonical relation URN"), required("rationale", "TEXT", "why the relation still holds against the new pins"),
			optional("id", "ID", "stable repin id; defaults to the next rN"),
			optional("from-revision", "URN", "exact source revision pin to advance to"), optional("to-revision", "URN", "exact target revision pin to advance to"),
			optional("from-content-digest", "DIGEST", "exact source design content digest to advance to"), optional("to-content-digest", "DIGEST", "exact target design content digest to advance to"),
			requestIDFlag, jsonFlag,
		},
		Positionals: sagaOnly,
	},
