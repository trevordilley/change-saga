...
func TestV5ContractDoesNotEnableRuntimeSupport(t *testing.T) {
	if SupportedSagaVersion(5) || SagaSchemaURL(5) != "" {
		t.Fatal("phase 0 must not enable v5 production readers or writers")
	}
	root := filepath.Join(t.TempDir(), "v5.saga")
	writeTestFile(t, filepath.Join(root, "saga.json"), `{"$schema":"https://changesaga.dev/schema/v5/saga.schema.json","version":5,"id":"v5","title":"Contract only","source":{"repository":"https://example.test/acme/app.git","base":"main","head":"feature"}}`)
	...
	writeTestFile(t, filepath.Join(root, "overview.fragment", "content.md"), "Contract only.\n")
	_, validation, err := Load(root)
	if err != nil {
		t.Fatal(err)
	}
	if validation.Valid {
		t.Fatal("production loader unexpectedly accepted v5")
	}
	found := false
	for _, issue := range validation.Issues {
		found = found || strings.Contains(issue.Message, "unsupported Saga version 5; expected 2, 3, or 4")
	}
	if !found {
		t.Fatalf("unsupported-version diagnostic missing: %#v", validation.Issues)
	}
}
