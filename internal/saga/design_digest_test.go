...
func TestCurrentDesignContentDigestsTrackAuthoredContentOnly(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, testFeatureDir, "overview.fragment", "content.md"), "Narrative.\n")
	writeTestFile(t, filepath.Join(root, testFeatureDir, "___design", "architecture.chapter", "chapter.json"), `{"version":2,"id":"architecture","title":"Architecture"}`)
	fragmentDir := filepath.Join(root, testFeatureDir, "___design", "architecture.chapter", "flow.fragment")
...
func loadVisualDigestFixture(t *testing.T) (*Saga, visualDigestFixture) {
	...
	bundle := filepath.Join(root, testFeatureDir, EmbeddedSlidesDir, "architecture.deck")
