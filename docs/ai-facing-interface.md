...
change-saga query persona-references --saga PATH --persona ID|URN [--cursor TOKEN] [--limit N] [--conflict-cursor TOKEN] [--conflict-limit N]
change-saga query terms          --saga PATH [--term ID|URN] [--story ID|URN] [--ref LOCATION] [--cursor TOKEN] [--limit N]
change-saga query term-references --saga PATH --term ID|URN [--cursor TOKEN] [--limit N] [--conflict-cursor TOKEN] [--conflict-limit N]
```
...
Unresolved conflicted owners use the separately bounded
`data.completeness.unresolved_page`; follow it with `--conflict-cursor` even
when the primary reference page is empty. These operations exclude prose, SVG
text, history, and transitive expansion. `terms`
