...
contract and routes agents to focused references for diagrams, stories, terms,
integration/recovery, CI, query navigation, and occasional format lookup. It no
longer requires every authoring task to preload the combined general, query,
format, and deck-authoring manuals.
...
branches. New sibling CLI/query work should be integrated only after its public
contract lands, in the single routed reference that owns that workflow.
...
`format.md`, and 2,801 in `authoring.md` (9,693 combined). The revised
entrypoint has 1,148 words. Routed references have: query 1,134; diagrams
1,329; stories 579; terms 536; integration 741; format 267; and CI 662.

| Realistic task | Baseline mandatory words | Revised mandatory words |
| --- | ---: | ---: |
| Inspect an existing Saga | 5,613 | 2,282 |
| Author a diagram/deck with exact evidence | 9,693 | 3,611 |
| Add or revise a sourced story | 6,892 | 2,861 |
| Revise project vocabulary | 6,892 | 2,818 |
| Reconcile and hand off a comparison without visual edits | 6,892 | 3,023 |
| Reconcile a comparison and revise its visual artifact | 9,693 | 4,352 |
...
The implementation adds a packaging-level routing test that installs the
embedded skill into a temporary fixture and exercises realistic diagram,
story, term, integration/recovery, and CI requests. It verifies that each route
ships its required files and does not preload unrelated domain references. A
negative assertion ensures the former blanket `references/authoring.md` is not
present.
