...
test("a Report Saga opens several implementation decks without paginating its documentation", async ({ page, saga }) => {
  ...
  await page.getByRole("link", { name: "Request flow", exact: true }).click();

  const slidePanel = page.locator("#view-slides");
  await expect(slidePanel).toBeVisible();
  await expect(slidePanel.locator("[data-native-slide]")).toHaveCount(2);
  await expect(slidePanel.locator('[data-native-slide][data-slide-title="Request flow"]')).toBeVisible();
  ...
  await page.getByRole("link", { name: "Failure path", exact: true }).click();
